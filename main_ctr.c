...
int main() {
    ...
    print_time(1);
    if (!DRBG_CTR_AES256_conf(&conf)) {
        printf("Conf err");
        return 1;
    }

    print_time(2);
    if (!DRBG_CTR_new(&drbg, &conf, true)) {
        printf("New err");
        return 2;
    }
    ...
    print_time(4);
    if (!DRBG_CTR_instantiate(&drbg, ent, ENTROPY_LEN, NULL, 0, serial, 16)) {
    ...
    print_time(5);
    for (int j = 0; j < 10; ++j) {
        if (!DRBG_CTR_generate(&drbg, serial, 16, res, DRBG_LEN)) {
            printf("Gen err");
            return 4;
        } else {
    ...
    print_time(7);
    if (!DRBG_CTR_reseed(&drbg, ent, ENTROPY_LEN, serial, 16)) {
        printf("Reseed err");
        return 5;
    }
    ...
    print_time(9);
    if (!DRBG_CTR_uninstantiate(&drbg)) {
